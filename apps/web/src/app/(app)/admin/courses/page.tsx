...
export default function AdminCoursesPage() {
  ...
  function goBack(target: Level) {
    ...
    if (target === "departments") {
      setSelectedDept(null); setSelectedYear(null);
      setSelectedSemester(null); setSelectedCourse(null);
    }
    if (target === "years") {
      setSelectedYear(null); setSelectedSemester(null); setSelectedCourse(null);
    }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm flex-wrap">
        <button onClick={() => goBack("departments")} className={cn("transition-colors", level === "departments" ? "text-primary font-medium" : "text-muted hover:text-secondary")}>
          Departments
        </button>
      ...
      {/* Add new row — hidden at materials level */}
      {level !== "materials" && (
        <div className="flex flex-col gap-2">
          ...
          {/* Teacher assignment for new courses */}
        ...
        <div className="bg-surface border border-default rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-2">
            {(["PDF", "IMAGE", "YOUTUBE"] as const).map((t) => (
              <button key={t} onClick={() => setMaterialType(t)} className={cn("px-3 py-1.5 rounded-lg text-xs font-medium transition-colors", materialType === t ? "bg-accent text-white" : "bg-elevated text-secondary hover:text-primary")}>{t}</button>
            ))}
          </div>
          <input value={materialTitle} onChange={(e) => setMaterialTitle(e.target.value)} placeholder="Material title (e.g. Chapter 3 — Brachial Plexus)" className="w-full bg-elevated border border-default rounded-xl px-4 py-2.5 text-sm text-primary placeholder:text-muted focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all" />
          ...
          <button onClick={handleUploadMaterial} disabled={uploading || !materialTitle.trim()} className="flex items-center gap-1.5 px-4 py-2.5 bg-accent hover:bg-accent-hover text-white rounded-xl text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={level} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.2 }} className="space-y-0">
              ...
              {level === "departments" && departments.map((dept, i) => (
                <button key={dept.id} onClick={() => openDepartment(dept)} className={cn("w-full flex items-center gap-3 py-3.5 px-2 -mx-2 rounded-lg text-left hover:bg-elevated transition-colors duration-150", i < departments.length - 1 && "border-b border-subtle")}>
              ...
              {level === "years" && years.map((year, i) => (
                <button key={year.id} onClick={() => openYear(year)} className={cn("w-full flex items-center gap-3 py-3.5 px-2 -mx-2 rounded-lg text-left hover:bg-elevated transition-colors duration-150", i < years.length - 1 && "border-b border-subtle")}>
                  <div className="flex-1 min-w-0">
              ...
              {level === "semesters" && semesters.map((sem, i) => (
                <button key={sem.id} onClick={() => openSemester(sem)} className={cn("w-full flex items-center gap-3 py-3.5 px-2 -mx-2 rounded-lg text-left hover:bg-elevated transition-colors duration-150", i < semesters.length - 1 && "border-b border-subtle")}>
                  <div className="flex-1 min-w-0">
              ...
              {level === "courses" && courses.map((course, i) => (
                <div key={course.id} className={cn("py-3.5 px-2 -mx-2 rounded-lg hover:bg-elevated transition-colors duration-150", i < courses.length - 1 && "border-b border-subtle")}>
                  <div className="flex items-center gap-3">
                    <button onClick={() => openCourse(course)} className="flex items-center gap-3 flex-1 min-w-0 text-left">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted">
                          {course.teacher?.fullName ?? "No teacher assigned"} · {course.isPublished ? "Published" : "Draft"}
                        </p>
                      </div>
                    </button>
                    ...
                    {assigningCourseId === course.id ? (
                      <select
                        autoFocus
                        defaultValue={course.teacher?.id ?? ""}
                    ...
                    <ChevronRight className="h-4 w-4 text-muted flex-shrink-0" />
                    <button onClick={() => handleArchiveCourse(course.id)} className="text-muted hover:text-error transition-colors p-1.5" title="Archive course">
              ...
              {level === "materials" && materials.map((m, i) => {
                ...
                return (
                  <div key={m.id} className={cn("flex items-center gap-3 py-3.5 px-2 -mx-2 rounded-lg hover:bg-elevated transition-colors duration-150", i < materials.length - 1 && "border-b border-subtle")}>
                    ...
                    <button onClick={() => handleDeleteMaterial(m.id)} className="text-muted hover:text-error transition-colors p-1.5" title="Delete material">
              ...
              {!loading && (
                ...
                <p className="text-sm text-muted text-center py-12">
                  {level === "materials" ? "No materials uploaded yet — use the form above." : "Nothing here yet — add the first one above."}
                </p>
              )}
            </motion.div>
          </AnimatePresence>
