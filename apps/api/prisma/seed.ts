...
async function main() {
  ...
  if (!adminEmail || !adminPassword) {
    throw new Error(
      'SUPER_ADMIN_EMAIL and SUPER_ADMIN_PASSWORD must be set before seeding',
    );
  }
  ...
  // Created idempotently from explicit env vars. On re-runs we do NOT
  // overwrite an existing password, so a rotated credential survives reseeding.
